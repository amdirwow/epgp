...
do
  ...
  local function OnMouseOverTooltip(anchorObject, name, desc, usage)
    ...
    if type(usage) == "string" then
      GameTooltip:AddLine("Використання: "..usage, NORMAL_FONT_COLOR.r, NORMAL_FONT_COLOR.g, NORMAL_FONT_COLOR.b, 1)
    end
    GameTooltip:Show()
  end
  ...
  local function OnFallbackDropdownTooltip(self, event)
    OnMouseOverTooltip( self.frame,
                        'Сумісність fallback для старих клієнтів',
                        'Якщо в рейді є гравці зі старими клієнтами, тут можна вказати, куди саме потрапить їхній вибір у вашому наборі кнопок. Кожне значення можна використати лише один раз.',
                        'Наприклад, якщо виставити тут Мейнспек, старі клієнти, які вибирають mainspec у своєму вікні, потраплять саме в цю категорію кнопки.')
  end
  
  local function OnGPEditTooltip(self, event)
    OnMouseOverTooltip( self.frame,
                        'Перевизначення значення GP',
                        'Заповніть це поле, щоб змінити GP, коли гравець натискає цю кнопку. Це лише додає варіант у вікно розподілу майстра здобичі, остаточне рішення все одно за ним.',
                        "\r\nПорожньо: використовувати звичайне значення GP"..
                        "\r\n50%: використовувати 50% від звичайного GP"..
  ...
  local function OnEditCaptionTooltip(self, event)
    OnMouseOverTooltip( self.frame,
                        'Підпис кнопки',
                        'Це поле задає текст на кнопці. Він відображається у вікнах вибору та моніторингу.')
  end
  ...
  local function OnEditCaptionTooltip(self, event)
    OnMouseOverTooltip( self.frame,
                        'Підпис кнопки',
                        'Це поле задає текст на кнопці. Він відображається у вікнах вибору та моніторингу.',
                        'Можна використовувати до 18 символів (без ^ ; *)')
  end
  
  local function OnColorTooltip(self, event)
    OnMouseOverTooltip( self.frame,
                        'Колір тексту моніторингу',
                        'Це поле задає колір тексту вибору у вікнах моніторингу.')
  end
