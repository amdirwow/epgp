...
function LootMasterML:GetFrame()
    ...
    titleFrame:SetScript("OnEnter", function() self:ShowInfoPopup("amdir EPGP LootMaster", "Натисніть і перетягніть, щоб перемістити це вікно.", "Подвійний клік згортає або розгортає це вікно.") end)
	...
	titletext:SetText( string.format("amdir EPGP LootMaster %s", self:GetVersionString() ) )
	...
	btnAnnounce:SetScript("OnClick", function()
            if not frame.currentLoot then return message('предмет не вибрано') end
            self:AnnounceLoot( frame.currentLoot.id )
            btnAnnounce:Hide();
    end)
    btnAnnounce:SetScript("OnEnter", function() self:ShowInfoPopup( "Оголосити",
                                                                    "Натисніть, щоб оголосити цей предмет усім кандидатам",
                                                                    "У них відкриється вікно вибору предмета.") end)
    ...
    btnDiscard:SetScript("OnEnter", function() self:ShowInfoPopup( "Прибрати предмет",
                                                                   "Натисніть, щоб видалити цей предмет і всі відповіді кандидатів зі списку.",
                                                                   "Використовуйте це, якщо не хочете розподіляти цей предмет і хочете закрити вікно LootMaster.") end)
...
function LootMasterML:DisplayLoot( item )
    ...
    if data.gpvalue2 and data.gpvalue2~='' then
        gp2 = format(' або %s', data.gpvalue2)
    end

    self.frame.lblInfo:SetText(format("Рівень предмета: %s, GP: %s%s%s", data.ilevel or -1, data.gpvalue or -1, gp2, binding or ''));
    ...
    if data.mayDistribute then
        ...
        self.frame.lblNoDistribute:SetText(format("** МОНІТОРИНГ ** Лише %s може розподіляти цей предмет **", tostring(data.lootmaster)))
    end
...
function LootMasterML:CandidateDropDownInitialize( frame, level, menuList )
    ...
    if UIDROPDOWNMENU_MENU_LEVEL == 1 then
        ...
        info.disabled = false;
        info.text = 'Шепіт';
        info.tooltipTitle = 'Шепіт'
        info.tooltipText = 'Надіслати повідомлення вибраному кандидату.'
        ...
        if loot.mayDistribute then
            ...
            if not loot.manual then
                ...
                info.text = format('Видати предмет і нарахувати %s GP', loot.gpvalue_manual or 0);
                info.tooltipTitle = info.text;
                info.tooltipText = format('Спробує видати предмет кандидату й нарахувати йому %s GP', loot.gpvalue_manual or 0);
                ...
                if loot.gpvalue2 and loot.gpvalue2~=0 then
                    info.isTitle = false;
                    info.disabled = not CanEditOfficerNote();
                    info.text = format('Видати предмет і нарахувати %s GP (100%%)', loot.gpvalue2 or 0);
                    info.tooltipTitle = info.text;
                    info.tooltipText = format('Спробує видати предмет кандидату й нарахувати йому %s GP', loot.gpvalue2 or 0);
                ...
                for i=1, loot.numButtons do
                    ...
                    if btn.gpValue then
                      ...
                      info.isTitle = false;
                      info.disabled = not CanEditOfficerNote();
                      info.text = format('Видати предмет і нарахувати %s GP за %s (%s)', gp, btn.text, vs);
                      info.tooltipTitle = info.text;
                      info.tooltipText = format('Спробує видати предмет кандидату й нарахувати %s GP за %s', gp, btn.text);
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = 'Видати предмет безкоштовно';
                info.tooltipTitle = 'Видати предмет безкоштовно';
                info.tooltipText = "Спробує видати предмет кандидату без нарахування GP.";
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = 'Видати на розпил';
                info.tooltipTitle = 'Видати на розпил';
                info.tooltipText = 'Спробує видати предмет кандидату для disenchant.';
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = 'Видати в банк';
                info.tooltipTitle = 'Видати в банк';
                info.tooltipText = 'Спробує видати предмет кандидату для зберігання в банку.';
                ...
                info.isTitle = false;
                info.disabled = false;
                info.text = '- Неможливо розподілити предмет -';
                info.tooltipTitle = info.text;
                info.tooltipText = "Цей предмет додано в список вручну. Розподіляти його доведеться вручну, а після завершення - прибрати зі списку."
            ...
            info.isTitle = false;
            info.disabled = false;
            info.text = '(Пере)оголосити предмет кандидату';
            info.tooltipTitle = '(Пере)оголосити предмет кандидату';
            info.tooltipText = 'Повторно відкриває вікно вибору предмета у кандидата після вильоту або дисконекту.';
            ...
            info.disabled = false;
            info.hasArrow = 1;
            info.text = 'Встановити відповідь вручну';
            info.tooltipTitle = 'Встановити відповідь вручну';
            info.tooltipText = 'Дозволяє вручну задати відповідь для цього кандидата';
            info.func = function() end;
    ...
    elseif UIDROPDOWNMENU_MENU_LEVEL==2 then
        ...
        if UIDROPDOWNMENU_MENU_VALUE == 'RESPONSE_OVERRIDE' then
            ...
            for i=1, loot.numButtons do
              ...
              info.tooltipText = 'Вручну встановлює відповідь кандидата на '..btn.text..'. Кандидат отримає whisper-повідомлення про це.'
              info.func = function()
            ...
            info.disabled = false
            info.text = 'Пас'
            info.tooltipTitle = 'Пас'
            info.tooltipText = 'Вручну встановлює для кандидата відповідь "пас". Кандидат отримає whisper-повідомлення про це.'
            info.func = function()
    ...
    info.isTitle = false;
    info.disabled = false;
    info.text = '-DEBUG- Прибрати предмет';
    info.tooltipTitle = nil;
    info.tooltipText = nil;
	info.func = function()
...
function LootMasterML:ShowGearInspectPopup( candidate, item )
    ...
    if not foundGear then
        self:ShowInfoPopup( candidate, 'Натисніть, щоб отримати поточне екіпірування.' );
    end
end
...
function LootMasterML:ShowNoteCellPopup( candidate, item )
    ...
    self:ShowInfoPopup('Нотатка від ' .. candidate .. ':', itemData or '');
end
...
function LootMasterML:SetCandidateResponseCellUserDraw( cell, self, candidate, item )
    ...
    if response == LootMaster.RESPONSE.DISENCHANT then
        if autoPass then
            ...
            text = format('Пас; Enchanter (%s)',self:GetCandidateData(item, candidate, "enchantingSkill") or 0);
        end
...
function LootMasterML:RaidInfoLookupActionClick(name)
    ...
    if self.raidinfoframe.rows[rowID].cols[3].value == 0 then
        SendChatMessage("Автоповідомлення: встановіть amdir EPGP LootMaster. Discord: https://discord.gg/jvMBqzsHa6  Site: https://amdir.in.ua/", "WHISPER", nil, name);
    end
end
...
function LootMasterML:RaidInfoLookupResponse(name, response)
    ...
    if iFound == 0 then
        ...
        row.cols[3].value = 'Not saved, available';
        row.color = {r=0,g=1,b=0,a=1}
    else
        -- Instance was found on the player's list
        if iResets <= 0 then
            row.cols[3].value = 'Raid lock expired, available';
            row.color = {r=0,g=1,b=0,a=1}
        else
            if frame.localInstanceID and iID == frame.localInstanceID then
              row.cols[3].value = 'Saved to your instance';
              row.color = {r=0,g=1,b=0,a=1}
            else
              if iFound == 2 then
                  row.cols[3].value = 'Saved ('..iID..'), mode differs';
                  row.color = {r=1,g=0.82,b=0,a=1}
              elseif iLocked == 'true' then
                  row.cols[3].value = 'Saved to '..iID..', locked';
                  row.color = {r=1,g=0,b=0,a=1}
              else
                  row.color = {r=1,g=0.5,b=0,a=1}
                  row.cols[3].value = 'Saved to '..iID..', but not yet locked';
              end
            end
        end
    end
...
function LootMasterML:ShowRaidInfoLookup()
    ...
    if not frame then
        ...
        btnInvite:SetText("Запросити вибраних")
        ...
        btnWhisper:SetScript("OnClick", function()
			...
			if strlen(strtrim(frame.tbWhisperBox:GetText())) == 0 then
				print("Введіть повідомлення в текстове поле")
				return
			end
			for i,row in pairs(rows) do
				local player = row.cols[2].value
        ...
        btnWhisper:SetText("Написати вибраним")
