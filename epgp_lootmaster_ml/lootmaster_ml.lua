...
LootMasterML	      = LibStub("AceAddon-3.0"):NewAddon("LootMasterML", "AceConsole-3.0", "AceComm-3.0", "AceEvent-3.0", "AceTimer-3.0")
...
function LootMasterML:OnInitialize()
    ...
    if IsInGuild() and CanEditOfficerNote() and not EPGP then
        StaticPopupDialogs["EPGPLM_EPGP_NOTINSTALLED"] = {
            text = "amdir EPGP LootMaster\r\n\r\n|cFFFF8080Увага:|r LootMaster увімкнений, але EPGP вимкнено.\r\n\r\nПереконайтеся, що EPGP встановлений і активний. Інакше GP за предмети не буде нараховуватися.\r\n\r\nІнші функції LootMaster, такі як анонс і розподіл луту, працюватимуть і без EPGP.",
            button1 = OKAY,
        ...
        if itemID then
            if self:IsCandidate( itemID, sender ) then
                if LootMaster.db.profile.ignoreResponseCorrections and tonumber(self:GetCandidateData( itemID, sender, 'response' )) >= LootMaster.RESPONSE.NEED then
                    self:SendWhisperResponse(format('Ви вже зробили вибір для %s. Майстер здобичі вимкнув зміну відповіді.', message), sender);
                    return true;
                else
                    ...
                    self:SendWhisperResponse(format('Зареєстровано %s для %s', command, message), sender);
                end
            else
                self:SendWhisperResponse(format('Ви не є кандидатом на %s. Можливо, не брали участі в бою?', message), sender);
                return true;
            end
        else
            self:SendWhisperResponse(format('%s не знайдено у списку луту. Можливо, предмет уже забрали?', message), sender);
            return true;
        end

        return true;
    else
        self:SendWhisperResponse(format('Команду "%s" не зрозуміло. Використання: /w %s !epgp need/greed/pass [itemlink]', command, UnitName('player')), sender);
        self:Print(format('%s надіслав "%s %s"; команду не розпізнано, відправлено підказку.', sender or '', command or '', message or ''));
        return true;
    end
...
function LootMasterML:ChatFrameFilter(...)
    ...
    if command then
        ...
        -- Error?!
        if not cbOK then
            LootMasterML:Print(format("Помилка обробки повідомлення '%s' від %s: %s", tostring(params), tostring(sender), tostring(ret)))
...
function LootMasterML:GetVersionString()
    ...
    return (lm.GetVersionString(lm) or 'невідома версія')
end
...
function LootMasterML:AskCandidateIfNeeded( link, candidate )
	...
	if not loot then
		return self:Print(format("Не вдалося запитати гравця про предмет, бо %s відсутній у кеші", link))
	end
	...
	if candidate == 'RAID' or candidate == 'PARTY' then
        ...
        SendChatMessage(format('%sбудь ласка, напишіть мені !epgp need/greed/pass %s (або скористайтеся вікном, якщо у вас встановлений EPGPLootmaster)', MsgPrefix or '', loot.link or ''), candidate);
    ...
    elseif candidate ~= UnitName('player') then
        ...
        SendChatMessage(format('%sбудь ласка, напишіть мені !epgp need/greed/pass %s (або скористайтеся вікном, якщо у вас встановлений EPGPLootmaster)', MsgPrefix or '', loot.link or ''), 'WHISPER', nil, candidate);
...
function LootMasterML:SetManualResponse(loot, candidate, response)
    ...
    self:SendWhisperResponse(format('Ваш вибір для %s вручну змінено на %s.', loot.link or 'невідомий предмет', sResponse or 'невідомо'), candidate);
...
function LootMasterML:GiveLootToCandidate( link, candidate, lootType, gp )
    ...
    if not loot or not loot.id then
        return self:Print( format("Не вдалося видати %s для %s: предмет не знайдено в кеші", tostring(link), tostring(candidate) ) )
    end
	...
	if not slotID then
		return self:Print( format("Не вдалося видати %s для %s: слот здобичі не знайдено (вже забрали предмет або закрите вікно здобичі?)", tostring(loot.link), tostring(candidate) ) )
	end
	if not candidateID then
		return self:Print( format("Не вдалося видати %s для %s: кандидат не знайдений (офлайн або вийшов із групи?)", tostring(loot.link), tostring(candidate) ) )
	end
...
function LootMasterML:OnMasterLooterChange(masterlooter)
    ...
    if LootMaster.db.profile.use_epgplootmaster == 'enabled' then
        ...
        LootMaster:Print('Ви майстер здобичі, відстеження здобичі увімкнено');
        self:EnableTracking();
...
function LootMasterML:CHAT_MSG_LOOT( event, message )
    ...
    if lootType==LootMaster.LOOTTYPE.GP and lootGP~=0 then
        if CanEditOfficerNote() and EPGP and EPGP.IncGPBy then
            ...
            self:Print( format("<ПОМИЛКА> Не вдалося збільшити GP в офіцерській нотатці для %s %s (EPGP не встановлено або немає прав?)", sPlayer or 'невідомий гравець', sLink or 'невідомий предмет') )
        end
    end;
...
function LootMasterML:OPEN_MASTER_LOOT_LIST()
    ...
    for slot=1, numLootSlots do
        ...
        if slotItemID and slotItemID==itemID then
            ...
            if slotQ~=1 then
                self:Print( format("Не вдалося перерозподілити %s, тому що кількість ~= 1 (%s). Обробіть це вручну.", link, slotQ) )
                return;
            end
    ...
    -- Fallback
    if not itemID then
        self:Print( format('Не вдалося визначити кількість предмета для %s (itemid не знайдено)', tostring(link)) );
        totalQuantity = 1
    end

    -- Another sanity check... Check total quantity > 1
    if totalQuantity<1 then
        self:Print( format("Не вдалося перерозподілити %s, тому що загальна кількість < 1 (%s). Обробіть це вручну.", link, totalQuantity) )
		return;
    end
    ...
    if not lootID then return self:Print('Не вдалося зареєструвати предмет') end;
    ...
    if LootMaster.db.profile.AutoLootThreshold~=0 and LootMaster.db.profile.AutoLooter~='' and self.lootTable[lootID].autoLootable then
        ...
        if IsAltKeyDown() then
            self:Print('Автовидачу скасовано (виявлено alt+click)')
        else
            isAutoLooted = true
            if self:IsCandidate( lootID, LootMaster.db.profile.AutoLooter or '' ) then
                self:Print(format('Автоматично видаю %s для %s', link or 'nil', LootMaster.db.profile.AutoLooter or 'nil'))
                ...
                self:Print(format('Автовидача %s для %s не вдалася. Це не кандидат для цього предмета.', link or 'nil', LootMaster.db.profile.AutoLooter or 'nil'))
            end
        end
    end
    ...
    if autoAnnounce then
        if IsAltKeyDown() then
            self:Print('Автооголошення скасовано (виявлено alt+click)')
        else
