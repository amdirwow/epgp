...
function LootMaster:ShowUpdateFrame( sender, iVersion, sVersion )
    ...
    message(string.format("Повідомлення від %s: оновіть amdir EPGP LootMaster до актуальної версії. Discord: https://discord.gg/jvMBqzsHa6  Site: https://amdir.in.ua/", sender))
end
...
function LootMaster:InitUI()
    ...
    titletext:SetText(string.format("amdir EPGP LootMaster %s  |  discord.gg/jvMBqzsHa6  |  amdir.in.ua", self:GetVersionString()))
...
function LootMaster:EnterCombat()
    ...
    if self.frame and self.frame:IsShown() then
        self:Print("Ви увійшли в бій, інтерфейс приховано.");
...
function LootMaster:SaveNote( frame, keepVisibility )
    ...
    if frame.data.note and frame.data.note~='' then
        frame.lblNote:SetText(format('Моя нотатка: %s', frame.data.note or ''))
    else
...
function LootMaster:CreateLootSelectFrame()
    ...
    lblLooter:SetText("Майстер здобичі: -")
    ...
    b:SetScript("OnUpdate", function(o, elapsed)
        timeoutUpdateElapsed = timeoutUpdateElapsed + elapsed
        if timeoutUpdateElapsed < 0.2 then
            return
        end
        elapsed = timeoutUpdateElapsed
        timeoutUpdateElapsed = 0
        if not frame or not frame.data or not frame.data.timeoutLeft then 
            b:SetMinMaxValues(0,100); b:SetValue(100);
            frame.lblTimeout:SetText('Без таймера')
            return;
        end
        ...
        if frame.data.timeoutLeft<0 then
            ...
            b:SetValue(0);
            frame.lblTimeout:SetText('Час вийшов')
        ...
        frame.lblTimeout:SetText(format("%s c до автопасу", ceil(frame.data.timeoutLeft)))
...
function LootMaster:UpdateLootUI()
    ...
    for i, data in ipairs(self.lootList) do repeat
        ...
        lootFrame.lblInfo:SetText(format("Рівень предмета: %s, GP: %s", data.ilevel or -1, gp))
        lootFrame.lblLooter:SetText(format("Майстер здобичі: %s", data.lootmaster or 'невідомо'))
