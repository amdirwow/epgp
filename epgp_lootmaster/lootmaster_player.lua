...
function LootMaster:CommandReceived(prefix, message, distribution, sender)
    ...
    if autoPassClassList and binding=='pickup' then
        ...
        if playerClass and autoPassClassList[playerClass] then
            ...
            self:Print(format('Автопас для %s (персонаж не підходить)', link or 'невідомий предмет'));
  ...
  elseif command == 'GETRAIDINFO' then
    ...
    local fallbackInstanceID, fallbackInstanceReset, fallbackLocked
    local fallbackExtended, fallbackInstanceIDMostSig, fallbackIsRaid
    ...
    for i=1, n do
       ...
       if instanceName == searchName and searchDifficulty == instanceDifficulty and searchSize == maxPlayers then
          ...
          break
       elseif instanceName == searchName then
          instanceReset = tonumber(instanceReset) or 0
          if instanceReset > 0 and not fallbackInstanceID then
              fallbackInstanceID = instanceID
              fallbackInstanceReset = instanceReset
              fallbackLocked = locked
              fallbackExtended = extended
              fallbackInstanceIDMostSig = instanceIDMostSig
              fallbackIsRaid = isRaid
          end
       end
    end
    if not instanceFound and fallbackInstanceID then
      self:SendCommand('RAIDINFO', format('%s^2^%s^%s^%s^%s^%s^%s',
                                      message,
                                      tostring(fallbackInstanceID or ''),
                                      tostring(fallbackInstanceReset or ''),
                                      tostring(fallbackLocked or ''),
                                      tostring(fallbackExtended or ''),
                                      tostring(fallbackInstanceIDMostSig or ''),
                                      tostring(fallbackIsRaid or '')), sender)
    elseif not instanceFound then
...
function LootMaster:RegisterHeadCountLoot( player, link, lootTypeID, lootGP )
    ...
    if not itemID then return ' (Некоректне посилання)' end;

    if not HeadCount then return '' end

    local raidTracker = HeadCount:getRaidTracker();
    if not raidTracker then
        return ' (Не вдалося записати в HeadCount: raidTracker відсутній)'
    end

    local raid = raidTracker:retrieveMostRecentRaid()
    if not raid then
        return ' (Не вдалося записати в HeadCount: немає активного рейду)'
    end

    local lootList = raid:getLootList()
    if not lootList then
        return ' (Не вдалося записати в HeadCount: список предметів недоступний)'
    end

    local lastLoot = lootList[#lootList]
    if not lastLoot then
        return ' (Не вдалося записати в HeadCount: останній предмет не знайдено)'
    end

    if lastLoot:getItemId()~=strmatch(link, "item:(%d+):") then
        return ' (Не вдалося записати в HeadCount: itemID не знайдено)'
    end

    if lastLoot:getPlayerName()~=player then
        return ' (Не вдалося записати в HeadCount: предмет знайдено, але кандидат невірний)'
    end
    ...
    return ' (Предмет записано в HeadCount)'
end
...
function LootMaster:RegisterCTRaidTrackerLoot( player, link, lootTypeID, lootGP )
    ...
    if not CT_RaidTracker_GetCurrentRaid
        ...
        return ' (Не вдалося записати в CT_RaidTracker: рейд не розпочато)'
    end
    ...
    for index, data in ipairs( CT_RaidTracker_RaidLog[CT_RaidTracker_GetCurrentRaid]["Loot"] ) do
        if data.player == player and strsplit(':', data.item.id) == itemID then
            ...
            return ' (Предмет записано в CT_RaidTracker)'
        end
    end

    return ' (Предмет не записано в CT_RaidTracker; задайте це вручну)'
end
