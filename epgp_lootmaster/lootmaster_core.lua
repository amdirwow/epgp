...
LootMaster          = LibStub("AceAddon-3.0"):NewAddon("EPGPLootMaster", "AceConsole-3.0", "AceComm-3.0", "AceEvent-3.0", "AceTimer-3.0")

local version 	    = "1.0.0"
local dVersion 	    = "2010-01-14T00:26:40Z"
local iVersion	    = 3
local iVersionML	  = 11
local _G            = _G
...
function LootMaster:OnInitialize()
    ...
    self.RESPONSE = {
        { ["CODE"]      = "NOTANNOUNCED",   ["SORT"] =  100,  ["COLOR"] = {1,1,1},        ["TEXT"] = 'Ще не оголошено кандидатам' },
        { ["CODE"]      = "INIT",           ["SORT"] =  200,  ["COLOR"] = {1,0,0},        ["TEXT"] = 'Офлайн або LootMaster не встановлено?' },
        ...
        { ["CODE"]      = "OFFSPEC",        ["SORT"] =  700,  ["COLOR"] = {1,1,0.5},      ["TEXT"] = 'Оффспек' },
        { ["CODE"]      = "MINORUPGRADE",   ["SORT"] =  600,  ["COLOR"] = {0.2,0.7,0.2},  ["TEXT"] = 'Малий апгрейд' },
    ...
    self:Print(format('amdir EPGP LootMaster %s завантажено. %s | %s', version, 'https://discord.gg/jvMBqzsHa6', 'https://amdir.in.ua/'))
end
  ...
  elseif command=='raidinfo' or command=='ri' or command=='saved' or command=='lock' then
    ...
    elseif command=='emulate' then
        ...
        if not player or not item or player=='' or item=='' then
            self:Print('Використання: /lm emulate player [itemlink]')
            self:Print('Це локально емуляє повідомлення "Гравець отримує [предмет]". Зазвичай використовується, щоб обійти стару проблему з порталами в Naxx.')
        else
    ...
    elseif command=='add' or command=='announce' then
        ...
        if not LootMasterML then return self:Print('Не вдалося додати предмет: ML-модуль не активний') end
        ml = LootMasterML;

        local lootLink = strtrim(args or '');
        if not args or not lootLink or lootLink=='' then return self:Print(format('Використання: /lm %s [lootlink]', command)) end;
...
function LootMaster:OutputLog()
    ...
    StaticPopupDialogs["EPGP_LOGOUTPUT_POPUP"] = {
        text = 'amdir EPGP LootMaster: скопіюйте текст нижче й передайте його адміністрації amdir разом з описом проблеми.',
        button1 = nil,
        button2 = 'OK',
        timeout = 0,
        whileDead = 1,
