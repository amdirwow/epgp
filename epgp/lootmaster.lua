...
RESPONSES = {
  { CODE = "NOTANNOUNCED",   SORT = 1,    COLOR = {1,1,1},        TEXT = 'Не оголошено' },
  { CODE = "INIT",           SORT = 2,    COLOR = {1,0,0},        TEXT = 'Офлайн' },
  { CODE = "WAIT",           SORT = 3,    COLOR = {1,0.5,0},      TEXT = 'Триває вибір' },
  { CODE = "TIMEOUT",        SORT = 4,    COLOR = {1,0,1},        TEXT = 'Час вибору вичерпано' },
...
LOOTTYPES = {
  { CODE = "UNKNOWN",        TEXT = '%s отримав %s з невідомої причини%4$s.' },
  { CODE = "GP",             TEXT = '%s отримав %s за %s GP%s.' },
  { CODE = "DISENCHANT",     TEXT = '%s отримав %s на розпилення%4$s.' },
  { CODE = "BANK",           TEXT = '%s отримав %s у банк%4$s.' }
}
...
function mod:AddLoot(link, mayDistribute, quantity)
  ...
  if db.auto_loot_threshold~=0 and db.auto_loot_candidate and db.auto_loot_candidate~='' then
    if (not isBoP or isBoE) and itemRarity <= db.auto_loot_threshold then
...
function mod:AddMonitorLoot(itemLink, gpvalue, gpvalue2, quantity, autoPassClassesEncoded)
  local loot = self:AddLoot(itemLink, false, quantity)
  if not loot then return end
  loot.gpvalue = tonumber(gpvalue) or loot.gpvalue or 0
  loot.gpvalue2 = tonumber(gpvalue2) or loot.gpvalue2
  loot.autoPassClasses = self:DecodeAutoPassClasses(autoPassClassesEncoded)
  return loot.id
end
...
function mod:DecreaseLoot(itemLink)
  ...
  if self:MonitorMessageRequired(loot.id) then
    self:CallRPC('RAID', nil, 'DecreaseLoot', loot.id)
  end
end
...
function mod:AnnounceLoot(itemID)
  ...
  if not loot or loot.announced then return true end
  loot.announced = true
...
function mod:AddCandidate(itemID, candidate, randomRoll, initResponse)
  ...
  local class = EPGP:GetClass(candidate)
  if not class then
    local _, unitClass = UnitClass(candidate)
    class = unitClass
  end
  ...
  if self:IsRPC() then
    ...
    if loot.isBoP and loot.autoPassClasses and class and loot.autoPassClasses[class] then
...
function mod:IsCandidate(itemID, candidate)
  local loot = self:GetLoot(itemID)
  if not loot or not candidate or not loot.candidates then return false end
  return loot.candidates[candidate] ~= nil
end
...
function mod:SetCandidateData(itemID, candidate, name, value)
  local cData = self:GetCandidate(itemID, candidate)
  if not cData or not name then return end
  cData[name] = value
  return value
end
...
function mod:SetCandidateResponse(itemID, candidate, response)
  local loot = self:GetLoot(itemID)
  local cData = self:GetCandidate(itemID, candidate)
  if not loot or not cData then return end

  response = tonumber(response) or RESPONSES.INIT
  local old = tonumber(cData.response) or 0

  cData.response = response
  cData.responsesort = (RESPONSES[response] and RESPONSES[response].SORT) or response

  if old < RESPONSES.TIMEOUT and response >= RESPONSES.TIMEOUT then
    loot.numResponses = (loot.numResponses or 0) + 1
  elseif old >= RESPONSES.TIMEOUT and response < RESPONSES.TIMEOUT then
    loot.numResponses = math.max((loot.numResponses or 0) - 1, 0)
  end
...
function mod:SetCandidateManualResponse(itemID, candidate, response)
  return self:SetCandidateResponse(itemID, candidate, response)
end
...
function mod:GiveLootToCandidate(itemID, candidate, lootingType, gp)
  local candidateID, slotID
  local loot = self:GetLoot(itemID)

  if not loot or not loot.id then
    return self:Print(format("Could not send %s to %s, loot not found in cache", tostring(itemID), tostring(candidate)))
  end
  ...
  for sID = 1, GetNumLootItems() do
    local slotLink = GetLootSlotLink(sID)
    if slotLink and ItemUtils:ItemlinkToID(slotLink) == loot.id then
      slotID = sID
      break
    end
  end
  ...
  if not slotID then
    return self:Print(format("Could not send %s to %s, lootslotID not found (already looted or loot window closed?)", tostring(loot.link), tostring(candidate)))
  end
  ...
  if lootingType ~= LOOTTYPES.GP then
    gp = 0
  end

  self:SetCandidateData(loot.id, candidate, "lootType", lootingType or LOOTTYPES.UNKNOWN)
...
function mod:SendCommand(command, message, target)
  if not target then
    return self:Print("Не вдалося надіслати команду: не вказано ціль")
  end

  local payload = format("%s:%s", tostring(command), tostring(message))
  if target == "RAID" or target == "PARTY" or target == "GUILD" then
    return self:SendCommMessage("EPGPLootMasterC", payload, target, nil, "ALERT")
  end

  return self:SendCommMessage("EPGPLootMasterC", payload, "WHISPER", target, "ALERT")
end

function mod:UpdateUI(itemLink)
  local loot = self:GetLoot(itemLink)
  local ui = EPGP:GetModule("lootmaster_ui", true)
  if loot then
    callbacks:Fire("LootMasterLootCandidatesLoaded", loot)
  end
  if ui and ui.SetupItemButtons then
    ui:SetupItemButtons()
  end
end
...
function mod:OnLootMasterChange(event, newLootMaster)
  ...
  if db.use_lootmaster == 'enabled' then
    ...
    EPGP:Print('Ви майстер здобичі, відстеження луту увімкнено.')
    self:EnableTracking()
...
function mod:OPEN_MASTER_LOOT_LIST(event)
  ...
  if not itemID or itemID==0 then
    EPGP:Print(format('Не вдалося отримати itemid для %s', tostring(itemLink)))
    return
  end
  ...
  for slotNum, slotData in pairs(lootSlots) do
    ...
    if slotItemID and slotItemID==itemID then
      ...
      if slotData.quantity~=1 then
        EPGP:Print(format("Не вдалося перерозподілити %s: quantity != 1 (%s). Обробіть це вручну. Якщо проблема повторюється, зверніться в Discord: https://discord.gg/jvMBqzsHa6", itemLink, slotData.quantity))
        return
      end
  ...
  if totalQuantity<1 then
    EPGP:Print(format("Не вдалося перерозподілити %s: total quantity < 1 (%s). Обробіть це вручну. Якщо проблема повторюється, зверніться в Discord: https://discord.gg/jvMBqzsHa6", itemLink, totalQuantity))
    return
  end
  ...
  if db.auto_loot_threshold~=0 and db.auto_loot_candidate~='' and loot.autoLootable then
    ...
    if IsAltKeyDown() then
      EPGP:Print('Автовидачу скасовано (виявлено alt+click)')
    else
      isAutoLooted = true
      if self:IsCandidate(lootID, db.auto_loot_candidate) then
        EPGP:Print(format('Автоматично видаю %s для %s', tostring(itemLink), tostring(db.auto_loot_candidate)))
        ...
        EPGP:Print(format('Автовидача %s не вдалася. Гравець %s не є кандидатом на цей предмет.', tostring(itemLink), tostring(db.auto_loot_candidate)))
      end
    end
  end
  ...
  if autoAnnounce then
    if IsAltKeyDown() then
      EPGP:Print('Автооголошення скасовано (виявлено alt+click)')
    else
      Debug("auto announce")
...
function mod:OnPlayerReceivesLoot(event, player, itemlink, quantity)
  ...
  if lootType==LOOTTYPES.GP and lootGP then
    if EPGP:CanIncGPBy(itemlink, lootGP) then
      EPGP:IncGPBy(tostring(player) or "nil", tostring(itemlink) or "nil", lootGP)
    else
      self:Print(format("<ПОМИЛКА> Не вдалося збільшити GP в офіцерських нотатках для %s %s (EPGP не встановлено або немає прав?)", player or "невідомий гравець", itemlink or "невідомий предмет"))
    end
  end
  ...
  if GetNumRaidMembers()>0 or GetNumPartyMembers()>0 then
    ...
    self:SendCommand('LOOTED', format('%s^%s^%s^%s', player, itemlink, lootType, lootGP), 'PARTY')
  else
    self:Debug("send to candidates")
    for candidate, id in pairs(loot.candidates) do
      self:SendCommand('LOOTED', format('%s^%s^%s^%s', player, itemlink, lootType, lootGP), candidate)
    end
  end

  -- Update the candidates status to LOOTED when we have more than 1 item
  if loot.quantity>1 then
    self:SetCandidateData(itemlink, player, 'looted', true)
  end

  -- Since it's looted, remove it from the cache and update the UI
  self:RemoveLoot(itemlink)
  self:UpdateUI()
...
function mod:PerformUpgradeCheck()
  -- Legacy upgrade check disabled in the amdir fork.
end
