...
local function IsLootMasterHandled(player, itemLink)
  ...
  if LootMasterML and LootMasterML.GetLoot then
    local ok, loot = pcall(LootMasterML.GetLoot, LootMasterML, itemLink)
    if ok and loot and loot.mayDistribute then
      return true
    end
  end
  ...
  if EPGP.GetModule then
    local ok, lootmaster = pcall(EPGP.GetModule, EPGP, "lootmaster", true)
    if ok and lootmaster and lootmaster.GetLoot then
      local okLoot, loot = pcall(lootmaster.GetLoot, lootmaster, itemLink)
      if okLoot and loot and loot.mayDistribute then
        return true
      end
    end
  end
...
local function ShowPopup(player, item, quantity)
  ...
  if not itemName or not itemLink or not itemRarity then return end
...
local function LootReceived(event_name, player, itemLink, quantity)
  if IsRLorML() and CanEditOfficerNote() then
    ...
    if not itemRarity then return end
