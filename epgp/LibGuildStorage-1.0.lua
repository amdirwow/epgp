...
function lib:GetNote(name)
  ...
  local _, _, _, note = FindLiveGuildMember(name)
  return note
end
...
function lib:SetNote(name, note)
  ...
  if not e then
    index_live, rank_live, class_live, old_note_live = FindLiveGuildMember(name)
    if index_live then
      e = {
        rank = rank_live,
        class = class_live,
    ...
    if pending_note[name] then
      DEFAULT_CHAT_FRAME:AddMessage(
        string.format("Ignoring attempt to set note before flushing pending "..
                      ...
                      "Повідомте про цю помилку та дії, які до неї привели: "..
                      "https://discord.gg/jvMBqzsHa6",
                    tostring(name),
                    tostring(e.note),
      ...
      if index_live then
        GuildRosterSetOfficerNote(index_live, note)
        e.note = note
...
function lib:GetClass(name)
  ...
  local _, _, class = FindLiveGuildMember(name)
  return class
end
...
function lib:GetRank(name)
  ...
  local _, rank = FindLiveGuildMember(name)
  return rank
end
...
function lib:IsCurrentState()
  return state == "CURRENT" or ((GetNumGuildMembers(true) or 0) > 0)
end
...
function RefreshOnUpdate()
  if state == "CURRENT" or state == "REMOTE_FLUSHING" then
...
local function Frame_OnUpdate(self, elapsed)
  ...
  if num_members == 0 then return end

  if not index or index >= num_members then
    index = 1
  end
  ...
  local last_index = math.min(index + 50, num_members)
  Debug("Processing from %d to %d members", index, last_index)

  for i = index, last_index do
    local name, rank, _, _, _, _, public_note, officer_note, _, _, class = GetGuildRosterInfo(i)
    if name then
      local note = officer_note
      if note == nil then
        note = public_note
