...
local EPGPWEB = "https://amdir.in.ua/"
...
local function CreateEPGPFrame()
  ...
  t:SetText("amdir EPGP "..EPGP.version)
...
local function CreateEPGPLogFrame()
  ...
  local function UpdateLogButtons()
  ...
  GS.RegisterCallback(EPGPLogFrame, "StateChanged", UpdateLogButtons)
  scrollBar:SetScript(
    "OnVerticalScroll",
    function(self, value)
...
local function EPGPSideFrameGPDropDown_Initialize(dropDown)
  ...
  for i=1,GP:GetNumRecentItems() do
    ...
    info.func = function(self, arg1)
                  ...
                  if parent.UpdateGPButton then
                    parent:UpdateGPButton()
                  end
                end
...
local function AddGPControls(frame)
  ...
  function frame:UpdateGPButton()
    if EPGP:CanIncGPBy(UIDropDownMenu_GetText(dropDown), editBox:GetNumber()) then
      button:Enable()
    else
      button:Disable()
    end
  end
  editBox:SetScript("OnTextChanged", function() frame:UpdateGPButton() end)
  ...
  for i=1,#dungeons,4 do
    if dungeons[i+2] == 2 then
      ...
      info.func = function(self)
                    ...
                    if parent.UpdateEPControls then
                      parent:UpdateEPControls()
                    end
                  end
      UIDropDownMenu_AddButton(info)
    end
...
local function AddEPControls(frame, withRecurring)
  ...
  otherEditBox:SetScript(
    ...
    function(self)
      ...
      if frame.UpdateEPControls then
        frame:UpdateEPControls()
      end
    end)
  ...
  function frame:UpdateEPControls()
    EnabledStatus(button)
    if recurring then
      if EPGP:RunningRecurringEP() then
        recurring:Enable()
      else
        EnabledStatus(recurring)
      end
    end
  end

  editBox:SetScript("OnTextChanged", function() frame:UpdateEPControls() end)
...
local function CreateEPGPFrameStandings()
  ...
  local function UpdateDecayButton()
  ...
  GS.RegisterCallback(main, "StateChanged", UpdateDecayButton)
  ...
  function modeText:TextUpdate()
    local mode
    if UnitInRaid("player") then
      mode = "|cFFFF0000"..L["Raid"].."|r"
    else
      mode = "|cFF00FF00"..L["Guild"].."|r"
    end
  ...
  local function UpdateStandings()
    ...
    for i=1,#rowFrame.rows do
      ...
      if j <= numMembers then
        ...
        local c = RAID_CLASS_COLORS[EPGP:GetClass(row.name)] or NORMAL_FONT_COLOR
...
function mod:OnEnable()
  ...
  EPGPFrame:SetScript(
    ...
    function()
      GuildRoster()
      if EPGPScrollFrame and EPGPScrollFrame:GetParent() then
        local parent = EPGPScrollFrame:GetParent()
