...
local columns = {
  { text="C",          width=20,   name="class", align="CENTER",
    popup="Натисніть, щоб сортувати за класом.",
    OnCellEnter="CellCandidateInfoPopup", OnCellLeave="HideInfoPopup"},

  { text="Кандидат",   width=80,   name="candidate",
    popup="Натисніть, щоб сортувати за ім'ям кандидата.",
    OnCellEnter="CellCandidateInfoPopup", OnCellLeave="HideInfoPopup"},

  { text="Ранг",       width=70,   name="guildrank",
    popup="Натисніть, щоб сортувати за гільдійським рангом.",
    OnCellEnter="CellCandidateInfoPopup", OnCellLeave="HideInfoPopup"},

  { text="Статус",     width=90,   name="status",
    popup="Натисніть, щоб сортувати за відповіддю/статусом."},

  { text="EP",         width=55,   name="ep", align="RIGHT",
    popup="Натисніть, щоб сортувати за Effort Points (EP)."},

  { text="GP",         width=55,   name="gp", align="RIGHT",
    popup="Натисніть, щоб сортувати за Gear Points (GP)."},

  { text="PR",         width=55,   name="pr", align="RIGHT",
    popup="Натисніть, щоб сортувати за пріоритетом EPGP (PR)."},

  { text="Рол",        width=25,   name="roll", align="RIGHT",
    popup="Натисніть, щоб сортувати за випадковим roll."},

  { text="N",          width=20,   name="note", align="CENTER",
    popup="Натисніть, щоб сортувати за нотаткою кандидата."},

  { text="Екіп",       width=120,  name="equipment", align="CENTER",
    popup="Натисніть, щоб сортувати за різницею item level відносно поточного предмета."}
}
...
function mod:DisplayItemUserSelection(lootData)
  ...
  if not lmf then
    ...
    btn:SetScript("OnEnter", function()
      mod:ShowInfoPopup("Мейнспек", "Оберіть це, якщо предмет є суттєвим покращенням "..
                                     "для вашого основного спека і ви готові витратити GP. "..
                                     "Для малого покращення використовуйте кнопку малого апгрейду.")
    end)
    ...
    btn:SetHeight(25)
    btn:SetWidth(115)
    btn:SetText("Малий апгрейд")
    btn:SetScript("OnEnter", function()
      mod:ShowInfoPopup("Малий апгрейд", "Оберіть це, якщо предмет дає лише невелике покращення "..
                                        "для основного спека і ви готові поступитися ним тим, кому він потрібніший.")
    end)
    btn:SetScript("OnLeave", mod.HideInfoPopup)
    ...
    btn:SetText("Оффспек")
    btn:SetScript("OnEnter", function()
      mod:ShowInfoPopup("Оффспек", "Оберіть це, якщо предмет є покращенням для вашого другого спека.")
    end)
    btn:SetScript("OnLeave", mod.HideInfoPopup)
    ...
    btn:SetText("Пас")
    btn:SetScript("OnEnter", function()
      mod:ShowInfoPopup("Пас", "Оберіть це, якщо ви не хочете претендувати на предмет. "..
                                     "Його отримає інший гравець.")
    end)
    btn:SetScript("OnLeave", mod.HideInfoPopup)
    local btnPass = btn
...
function mod:CreateEPGPFrame()
  ...
  caption:SetText("amdir EPGP LootMaster " .. EPGP.version)
  f.caption = caption
...
function mod:UpdateRow(rowID, rowData, rowObj, rowNum)
  ...
  cells.guildrank:SetText(rowData.guildrank or "")
...
function mod:CreateTable(parent)
  ...
  for i, colData in ipairs(columns) do
    ...
    header:SetScript("OnEnter", function()
      mod:ShowInfoPopup("Сортування", colData.popup)
    end)
