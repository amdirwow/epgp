...
local function RefreshStandings(order, showEveryone)
  if not next(ep_data) and not next(main_data) then
    local snapshot = {}
    GS:Snapshot(snapshot)
    if snapshot.guild_info or (snapshot.roster_info and #snapshot.roster_info > 0) then
      BootstrapFromGuildStorage()
    end
    if not next(ep_data) and not next(main_data) then
      for _, entry in ipairs(snapshot.roster_info or {}) do
        local name = entry[1]
        if name then
          ep_data[name] = 0
          gp_data[name] = 0
...
local function GetBestGuildInfo()
  local info = GS:GetGuildInfo()
  if info and info ~= "" then
    return info
  end
  return GetGuildInfoText() or ""
end

local function FindLiveGuildMember(name)
  if not name then
    return
  end
  ...
  for i = 1, num_members do
    local member_name, rank, _, _, _, _, public_note, officer_note, _, _, class =
...
local function GetBestGuildNote(name)
  local note = GS:GetNote(name)
  if note ~= nil then
    return note
  end

  local live = FindLiveGuildMember(name)
  if not live then
    return
  end

  if live.officer_note ~= nil and live.officer_note ~= "" then
    return live.officer_note
  end

  return live.public_note
end
...
local function ParseGuildNote(callback, name, note)
  ...
  if ep then
    ...
    local main_note = GetBestGuildNote(note)
...
local function BootstrapFromGuildStorage()
  local snapshot = {}
  GS:Snapshot(snapshot)

  if snapshot.guild_info then
    ParseGuildInfo(nil, "-EPGP-\n" .. snapshot.guild_info .. "\n-EPGP-")
  else
    ParseGuildInfo(nil, GetBestGuildInfo())
  end

  for _, entry in ipairs(snapshot.roster_info or {}) do
    local name, _, note = unpack(entry)
    ParseGuildNote(nil, name, note or "")
  end

  if #(snapshot.roster_info or {}) == 0 then
    local num_members = GetNumGuildMembers(true) or 0
    for i = 1, num_members do
...
function EPGP:GetClass(name)
  return GS:GetClass(name) or (FindLiveGuildMember(name) or {}).class
end
...
function EPGP:GrantStarterEP(amount, reason)
  ...
  local awarded = {}
  for name, ep in pairs(ep_data) do
    local raw_gp = gp_data[name]
    if ep == 0 and raw_gp == 0 then
      EPGP:IncEPBy(name, reason, amount, true)
      awarded[name] = true
    end
  end
...
function EPGP:DumpDiagnostics(outputFunc)
  outputFunc = outputFunc or function(s) DEFAULT_CHAT_FRAME:AddMessage(s) end
  ...
  local function DumpNote(label, note)
    if note == nil then
      return label .. "=nil"
    elseif note == "" then
      return label .. "=''"
    else
      return label .. "=" .. tostring(note)
    end
  end

  outputFunc("=== amdir EPGP діагностика ===")
  outputFunc("GS current=" .. tostring(GS:IsCurrentState()) ..
             " inGuild=" .. tostring(IsInGuild()) ..
             " canEditON=" .. tostring(CanEditOfficerNote()))

  local guildInfo = GS:GetGuildInfo() or ""
  guildInfo = guildInfo:gsub("\r", "\\r"):gsub("\n", "\\n")
  outputFunc("GuildInfo=" .. guildInfo)

  local numGuildMembers = GetNumGuildMembers(true) or 0
  outputFunc("Live guild members=" .. tostring(numGuildMembers))
  for i = 1, math.min(numGuildMembers, 8) do
    local name, rank, _, _, _, _, public_note, officer_note, _, _, class =
      GetGuildRosterInfo(i)
    outputFunc(string.format(
      "LIVE[%d] name=%s rank=%s class=%s %s %s",
      i,
      tostring(name),
      tostring(rank),
      tostring(class),
      DumpNote("public", public_note),
      DumpNote("officer", officer_note)))
  end

  local snapshot = {}
  GS:Snapshot(snapshot)
  outputFunc("Snapshot guildInfo=" .. tostring(snapshot.guild_info or "nil"))
  outputFunc("Snapshot roster count=" .. tostring(#(snapshot.roster_info or {})))
  for i = 1, math.min(#(snapshot.roster_info or {}), 8) do
    local entry = snapshot.roster_info[i]
    local name, class, note = unpack(entry)
    local ep, gp = DecodeNote(note)
    outputFunc(string.format(
      "SNAP[%d] name=%s class=%s note=%s parsedEP=%s parsedGP=%s",
      i,
      tostring(name),
      tostring(class),
      tostring(note),
  ...
  outputFunc("core counts: ep_data=" .. Count(ep_data) ..
             " gp_data=" .. Count(gp_data) ..
             " main_data=" .. Count(main_data) ..
             " alt_data=" .. Count(alt_data) ..
  ...
  for name, note in pairs(ignored) do
    shown = shown + 1
    outputFunc(string.format("IGNORED[%d] %s => %s", shown, tostring(name), tostring(note)))
    if shown >= 8 then
      break
    end
  end

  local ok, membersOrErr = pcall(function() return self:GetNumMembers() end)
  outputFunc("GetNumMembers() ok=" .. tostring(ok) .. " value=" .. tostring(membersOrErr))
  if ok then
    for i = 1, math.min(membersOrErr or 0, 8) do
      local name = self:GetMember(i)
      local ep, gp, main = self:GetEPGP(name)
      outputFunc(string.format(
        "STANDING[%d] name=%s ep=%s gp=%s main=%s class=%s",
        i,
        tostring(name),
        tostring(ep),
        tostring(gp),
        tostring(main),
        tostring(self:GetClass(name))))
    end
  end

  outputFunc("global_config decay=" .. tostring(global_config.decay_p) ..
             " extras=" .. tostring(global_config.extras_p) ..
             " base_gp=" .. tostring(global_config.base_gp) ..
             " min_ep=" .. tostring(global_config.min_ep))
  outputFunc("db ready=" .. tostring(not not self.db) ..
             " profile ready=" .. tostring(not not (db and db.profile)) ..
             " sort=" .. tostring(db and db.profile and db.profile.sort_order) ..
             " show_everyone=" .. tostring(db and db.profile and db.profile.show_everyone))
  outputFunc("=== кінець діагностики ===")
end
...
function EPGP:OnEnable()
  ...
  GS.RegisterCallback(self, "StateChanged", HandleGuildStorageStateChanged)
